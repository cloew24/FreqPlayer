...
object Frequencies {
    ...
    /**
     * Presets shown in the info / picker sheet — everything *except* the
     * "off" (440 Hz, no pitch shift) entry. Ordered by ascending Hz so the
     * list reads as a tonal sweep.
     *
     * 432 sits alongside the nine Solfeggio frequencies because all ten
     * belong to the same alternative-tuning / holistic-healing family that
     * gives the app its purpose. 440 is excluded: it's the absence of a
     * pitch shift, not a frequency choice in its own right, and showing
     * it as the first item in a "healing frequencies" list muddies the
     * intent.
     */
    val HEALING: List<FrequencyPreset> = ALL.filterNot { it.isStandard }.sortedBy { it.hz }
