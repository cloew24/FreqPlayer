 ...
 * reacts (UI updates + a single PlaybackParameters jump that the deep
 * AudioTrack buffer in [DeepBufferRenderersFactory] absorbs cleanly). The
 * service never writes `PlaybackParameters` directly — the ViewModel owns
 * that mutation.
 */
class PlaybackService : MediaSessionService() {
    ...
    override fun onCreate() {
        ...
        // Push the current preset into session extras so any controller —
        // notification, lock-screen widget, Wear OS, system media output
        // switcher — can read it without inspecting our custom commands.
        // The flow runs forever; cancellation comes through [scope].
        scope.launch {
            SettingsStore(applicationContext).flow.collect { saved ->
                val extras = Bundle().apply {
                    putString(EXTRA_PRESET_ID, saved.presetId)
                }
    ...
    /**
     * PendingIntent that the system uses for the tap-to-open behaviour on
     * the notification body and the lock-screen artwork. Without this, the
     * notification has no `setSessionActivity` and tapping does nothing.
     */
    ...
    companion object {
        ...
        /** Read by external controllers (lock-screen widgets, Wear) from
         *  [MediaSession.getSessionExtras] to display the currently selected
         *  tuning preset alongside playback metadata. */
        const val EXTRA_PRESET_ID = "com.aelant.freqshift.EXTRA_PRESET_ID"
 ...
 * picks the change up via the DataStore flow and applies the new pitch in
 * one jump (the deep AudioTrack buffer absorbs the Sonic reconfiguration);
