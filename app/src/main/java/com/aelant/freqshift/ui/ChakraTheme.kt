...
/**
 * Chakra theme as a *dynamic* composition local. Must be
 * [compositionLocalOf] (not [staticCompositionLocalOf]) because the value
 * changes at runtime as the user picks new presets — static locals are
 * baked into the call sites at composition time and don't trigger recomposes
 * when the value changes, which is precisely the "stale theme" bug we hit.
 *
 * Default value covers the case where a composable reads [LocalChakra]
 * without a [ProvideChakra] above it (e.g. previews); it matches
 * [Frequencies.NONE] (the off preset, neutral grey).
 */
val LocalChakra = compositionLocalOf {
